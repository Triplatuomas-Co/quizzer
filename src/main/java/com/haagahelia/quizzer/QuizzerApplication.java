...
public class QuizzerApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
        ...
        Teacher templateTeacher = teacherRepository.findByUsername(TEMPLATE_TEACHER_USERNAME);
		...
		// Check if quizzes for this teacher already exist to avoid duplicates on restart
        if (quizRepository.findByTeacher(templateTeacher).isEmpty()) {
            for (int i = 0; i < 10; i++) {
                Quiz quiz = new Quiz(category, templateTeacher, i, "Sample Quiz Title " + i, "Description for sample quiz " + i, true);

                Question question = new Question("Sample Question " + i, "Description for sample question " + i, 1, quiz);

                Option option = new Option("Sample Option " + i, i % 2 == 0, question);

                question.setQuiz(quiz);
                if (quiz.getQuestions().stream().noneMatch(q -> q.getTitle().equals(question.getTitle()))) {
                    quiz.getQuestions().add(question);
                }
                option.setQuestion(question);
                if (question.getOptions().stream().noneMatch(o -> o.getText().equals(option.getText()))) {
